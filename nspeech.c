...
char **argv;
{
  ...
  printf("window %2d u_lo %3d u_hi %3d ref_lo %3d l_flag %d a_flag %d\n",
	 window, u_lo, u_hi, ref_lo, l_flag, a_flag);
  ...
  /* outer loop over all unknown utterances from u_lo to u_hi */
  for (u = u_lo; u < u_hi; u++) {
    tick(&sub_time);
    ...
    if (min_distance == INT_MAX) { /* still equal to INT_MAX after 32 comparisons... */
      no_match++;
      min_distance  = -1;          /* report -1 min_distance to stdout below... */
